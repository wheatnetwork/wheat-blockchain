...
# Gets the version from the result of "wheat version"
...
def get_wheat_version() -> str:
    version: str = "0.0"
    output = subprocess.run(["wheat", "version"], capture_output=True)
...
def update_version(package_json_path: str):
    ...
    data["version"] = get_wheat_version()
...
if __name__ == "__main__":
    update_version(f"{os.path.dirname(__file__)}/wheat-blockchain-gui/package.json")
    update_version(f"{os.path.dirname(__file__)}/wheat-blockchain-gui/packages/gui/package.json")
