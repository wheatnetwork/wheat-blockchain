...
from wheat.pools.pool_puzzles import SINGLETON_MOD_HASH, create_p2_singleton_puzzle
...
from wheat.types.signing_mode import SigningMode
...
from wheat.util.config import load_config, str2bool, selected_network_address_prefix
...
from wheat.wallet.singleton import create_singleton_puzzle
...
from wheat.wallet.util.compute_hints import compute_coin_hints
...
from wheat.wallet.util.transaction_type import CLAWBACK_TRANSACTION_TYPES, TransactionType
...
from wheat.wallet.wallet import CHIP_0002_SIGN_MESSAGE_PREFIX, Wallet
from wheat.wallet.wallet_coin_record import WalletCoinRecord
from wheat.wallet.wallet_coin_store import CoinRecordOrder, GetCoinRecords
...
class WalletRpcApi:
    ...
    def __init__(self, wallet_node: WalletNode):
        ...
        self.pool_nft_puzzle_hash: Dict[bytes32, bytes32] = {}
    ...
    async def get_offer_summary(self, request) -> EndpointResult:
        ...
        from wheat.wallet.util.puzzle_compression import decompress_object_with_puzzles
    ...
    async def nft_mint_bulk(self, request) -> EndpointResult:
        ...
        wheat_coins = None
        if wheat_coin_list:
            wheat_coins = set([Coin.from_json_dict(wheat_coin) for wheat_coin in wheat_coin_list])
        ...
        if mint_from_did:
            ...
            sb = await nft_wallet.mint_from_wheat(
                metadata_list,
                mint_number_start=mint_number_start,
                mint_total=mint_total,
                target_list=target_list,
    ...
    async def find_pool_nft(self, request) -> EndpointResult:
        ...
        try:
            ...
            if puzzle_hash is None:
                ...
                for puzzle_hash in list(puzzle_hashes):
                ...
                if program_puzzle is None:
                    return {"error": f"not find nft puzzle_hash"}
            else:
            ...
            return {"error": f"Exception from 'find_pool_nft' {e}"}
        finally:
            client.close()
    ...
    async def recover_pool_nft(self, request) -> EndpointResult:
        ...
        try:
            ...
            return {"error": f"Exception from 'recover_pool_nft' {e}"}
        finally:
            client.close()
        await client.await_closed()
        return data
