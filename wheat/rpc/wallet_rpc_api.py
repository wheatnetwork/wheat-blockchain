...
from wheat.consensus.block_rewards import calculate_base_farmer_reward
from wheat.data_layer.data_layer_errors import LauncherCoinNotFoundError
from wheat.data_layer.data_layer_wallet import DataLayerWallet
from wheat.pools.pool_puzzles import create_p2_singleton_puzzle, POOL_OUTER_MOD_HASH
from wheat.pools.pool_wallet import PoolWallet
from wheat.pools.pool_wallet_info import FARMING_TO_POOL, PoolState, PoolWalletInfo, create_pool_state
from wheat.protocols.protocol_message_types import ProtocolMessageTypes
from wheat.protocols.wallet_protocol import CoinState
from wheat.rpc.rpc_server import Endpoint, EndpointResult, default_get_connections
from wheat.rpc.util import tx_endpoint
from wheat.server.outbound_message import NodeType, make_msg
from wheat.server.ws_connection import WSWheatConnection
from wheat.simulator.simulator_protocol import FarmNewBlockProtocol
...
from wheat.types.signing_mode import CHIP_0002_SIGN_MESSAGE_PREFIX, SigningMode
from wheat.types.spend_bundle import SpendBundle, estimate_fees
from wheat.util.bech32m import decode_puzzle_hash, encode_puzzle_hash
from wheat.util.byte_types import hexstr_to_bytes
from wheat.util.config import load_config, str2bool
from wheat.util.errors import KeychainIsLocked
from wheat.util.ints import uint16, uint32, uint64
from wheat.util.keychain import bytes_to_mnemonic, generate_mnemonic
from wheat.util.misc import UInt32Range
from wheat.util.path import path_from_root
from wheat.util.streamable import Streamable, streamable
from wheat.util.ws_message import WsRpcMessage, create_payload_dict
from wheat.wallet.cat_wallet.cat_constants import DEFAULT_CATS
from wheat.wallet.cat_wallet.cat_info import CRCATInfo
from wheat.wallet.cat_wallet.cat_wallet import CATWallet
from wheat.wallet.cat_wallet.dao_cat_info import LockedCoinInfo
from wheat.wallet.cat_wallet.dao_cat_wallet import DAOCATWallet
from wheat.wallet.conditions import Condition
from wheat.wallet.dao_wallet.dao_info import DAORules
from wheat.wallet.dao_wallet.dao_utils import (
...
from wheat.wallet.dao_wallet.dao_wallet import DAOWallet
from wheat.wallet.derive_keys import (
    MAX_POOL_WALLETS,
...
from wheat.wallet.did_wallet import did_wallet_puzzles
from wheat.wallet.did_wallet.did_info import DIDCoinData, DIDInfo
from wheat.wallet.did_wallet.did_wallet import DIDWallet
from wheat.wallet.did_wallet.did_wallet_puzzles import (
    DID_INNERPUZ_MOD,
    did_program_to_metadata,
    match_did_puzzle,
    metadata_to_program,
)
from wheat.wallet.nft_wallet import nft_puzzles
from wheat.wallet.nft_wallet.nft_info import NFTCoinInfo, NFTInfo
from wheat.wallet.nft_wallet.nft_puzzles import get_metadata_and_phs
from wheat.wallet.nft_wallet.nft_wallet import NFTWallet
from wheat.wallet.nft_wallet.uncurry_nft import UncurriedNFT
from wheat.wallet.notification_store import Notification
from wheat.wallet.outer_puzzles import AssetType
from wheat.wallet.payment import Payment
from wheat.wallet.puzzle_drivers import PuzzleInfo, Solver
from wheat.wallet.puzzles.clawback.metadata import AutoClaimSettings, ClawbackMetadata
from wheat.wallet.puzzles.p2_delegated_puzzle_or_hidden_puzzle import puzzle_hash_for_synthetic_public_key
from wheat.wallet.singleton import (
    SINGLETON_LAUNCHER_PUZZLE_HASH,
    create_singleton_puzzle,
    get_inner_puzzle_from_singleton,
)
from wheat.wallet.trade_record import TradeRecord
from wheat.wallet.trading.offer import Offer
from wheat.wallet.transaction_record import TransactionRecord
from wheat.wallet.uncurried_puzzle import uncurry_puzzle
from wheat.wallet.util.address_type import AddressType, is_valid_address
from wheat.wallet.util.compute_hints import compute_spend_hints_and_additions
from wheat.wallet.util.compute_memos import compute_memos
from wheat.wallet.util.query_filter import HashFilter, TransactionTypeFilter
from wheat.wallet.util.transaction_type import CLAWBACK_INCOMING_TRANSACTION_TYPES, TransactionType
from wheat.wallet.util.tx_config import DEFAULT_TX_CONFIG, CoinSelectionConfig, CoinSelectionConfigLoader, TXConfig
from wheat.wallet.util.wallet_sync_utils import fetch_coin_spend_for_coin_state
from wheat.wallet.util.wallet_types import CoinType, WalletType
from wheat.wallet.vc_wallet.cr_cat_drivers import ProofsChecker
from wheat.wallet.vc_wallet.cr_cat_wallet import CRCATWallet
from wheat.wallet.vc_wallet.vc_store import VCProofs
from wheat.wallet.vc_wallet.vc_wallet import VCWallet
from wheat.wallet.wallet import Wallet
from wheat.wallet.wallet_coin_record import WalletCoinRecord
from wheat.wallet.wallet_coin_store import CoinRecordOrder, GetCoinRecords, unspent_range
from wheat.wallet.wallet_info import WalletInfo
from wheat.wallet.wallet_node import WalletNode
from wheat.wallet.wallet_protocol import WalletProtocol
...
class WalletRpcApi:
    ...
    pool_nft_puzzle_hash: Dict[bytes32, bytes32] = {}

    def __init__(self, wallet_node: WalletNode):
        assert wallet_node is not None
        self.service = wallet_node
        self.service_name = "wheat_wallet"
    ...
    async def get_latest_singleton_coin_spend(
        self, peer: WSWheatConnection, coin_id: bytes32, latest: bool = True
    ...
    async def _check_key_used_for_rewards(
        ...
        farmer_target = config["farmer"].get("wheat_target_address")
        pool_target = config["pool"].get("wheat_target_address")
    ...
    async def create_new_wallet(
        ...
        if request["wallet_type"] == "cat_wallet":
            ...
            if request["mode"] == "new":
                if request.get("test", False):
                    ...
                    raise ValueError(
                        ...
                        " Please use the CAT Admin Tool @ https://github.com/Wheat-Network/CAT-admin-tool instead."
                    )
            ...
            if request["mode"] == "new":
                ...
                from wheat.pools.pool_wallet_info import initial_pool_state_from_dict
    ...
    async def verify_signature(self, request: Dict[str, Any]) -> EndpointResult:
        ...
        # signatures made by `wheat keys sign`, which uses BLS_MESSAGE_AUGMENTATION_HEX_INPUT
    ...
    async def get_offer_summary(self, request: Dict[str, Any]) -> EndpointResult:
        ...
        from wheat.util.bech32m import bech32_decode, convertbits
        from wheat.wallet.util.puzzle_compression import OFFER_MOD_OLD, decompress_object_with_puzzles
    ...
    async def cancel_offers(
        ...
        if cancel_all:
            ...
            asset_id = request.get("asset_id", "wheat")
        ...
        key = None
        if asset_id is not None and asset_id != "wheat":
    ...
    async def did_get_info(self, request: Dict[str, Any]) -> EndpointResult:
        ...
        return {
            ...
            "p2_address": encode_puzzle_hash(p2_puzzle.get_tree_hash(), AddressType.WHEAT.hrp(self.service.config)),
    ...
    async def nft_mint_bulk(
        ...
        wheat_coin_list = request.get("wheat_coins", None)
        wheat_coins = None
        if wheat_coin_list:
            wheat_coins = {Coin.from_json_dict(wheat_coin) for wheat_coin in wheat_coin_list}
        wheat_change_target = request.get("wheat_change_target", None)
        if wheat_change_target is not None:
            if wheat_change_target[:2] == "wheat":
                wheat_change_ph = decode_puzzle_hash(wheat_change_target)
            else:
                wheat_change_ph = bytes32(hexstr_to_bytes(wheat_change_target))
        else:
            wheat_change_ph = None
        ...
        if mint_from_did:
            txs = await nft_wallet.mint_from_did(
                ...
                wheat_coins=wheat_coins,
                wheat_change_ph=wheat_change_ph,
            ...
            txs = await nft_wallet.mint_from_wheat(
                metadata_list,
                mint_number_start=mint_number_start,
                mint_total=mint_total,
                target_list=target_list,
