...
from wheat.util.ssl_check import DEFAULT_PERMISSIONS_CERT_FILE, DEFAULT_PERMISSIONS_KEY_FILE
...
def get_wheat_ca_crt_key() -> Tuple[Any, Any]:
    crt = pkg_resources.resource_string(__name__, "wheat_ca.crt")
    key = pkg_resources.resource_string(__name__, "wheat_ca.key")
    return crt, key
...
def generate_ca_signed_cert(ca_crt: bytes, ca_key: bytes, cert_out: Path, key_out: Path):
    ...
    new_subject = x509.Name(
        [
            x509.NameAttribute(NameOID.COMMON_NAME, "Wheat"),
            x509.NameAttribute(NameOID.ORGANIZATION_NAME, "Wheat"),
    ...
    cert = (
        ...
        .add_extension(
            x509.SubjectAlternativeName([x509.DNSName("wheatcoin.top")]),
            critical=False,
        )
...
def make_ca_cert(cert_path: Path, key_path: Path):
    ...
    subject = issuer = x509.Name(
        [
            x509.NameAttribute(NameOID.ORGANIZATION_NAME, "Wheat"),
            x509.NameAttribute(NameOID.COMMON_NAME, "Wheat CA"),
            x509.NameAttribute(NameOID.ORGANIZATIONAL_UNIT_NAME, "Organic Farming Division"),
        ]
    )
    root_cert = (
...
def create_all_ssl(
    ...
    *,
    ...
    write_ssl_cert_and_key(wheat_ca_crt_path, wheat_ca_crt, wheat_ca_key_path, wheat_ca_key, overwrite=overwrite)
...
def main():
    return make_ca_cert(Path("./wheat_ca.crt"), Path("./wheat_ca.key"))
