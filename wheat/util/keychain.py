...
DEFAULT_USER = f"user-wheat-{CURRENT_KEY_VERSION}"  # e.g. user-wheat-1.8
DEFAULT_SERVICE = f"wheat-{DEFAULT_USER}"  # e.g. wheat-user-wheat-1.8
...
def supports_keyring_passphrase() -> bool:
    # Support can be disabled by setting WHEAT_PASSPHRASE_SUPPORT to 0/false
    return os.environ.get("WHEAT_PASSPHRASE_SUPPORT", "true").lower() in ["1", "true"]
...
def obtain_current_passphrase(prompt: str = DEFAULT_PASSPHRASE_PROMPT, use_passphrase_cache: bool = False) -> str:
    ...
    before failing.
    """
    from wheat.cmds.passphrase_funcs import prompt_for_passphrase
...
class Keychain:
    ...
    def migration_checked_for_current_version() -> bool:
        ...
        if migration_version_file.exists():
            current_version_str = pkg_resources.get_distribution("wheat-blockchain").version
