...
ROOT = pathlib.Path(importlib.import_module("wheat").__file__).absolute().parent.parent
...
def solve_name_collision_problem(analysis):
    ...
    for data in analysis.datas:
        if str(data[0]).startswith("wheat/"):
            zipped.append(data)
        else:
            datas.append(data)
...
version_data = copy_metadata(get_distribution("wheat-blockchain"))[0]
...
add_binary("wheat", f"{ROOT}/wheat/cmds/wheat.py", COLLECT_ARGS)
add_binary("daemon", f"{ROOT}/wheat/daemon/server.py", COLLECT_ARGS)

for server in SERVERS:
    add_binary(f"start_{server}", f"{ROOT}/wheat/server/start_{server}.py", COLLECT_ARGS)
