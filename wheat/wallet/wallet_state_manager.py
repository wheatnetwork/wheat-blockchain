...
from wheat.consensus.block_rewards import calculate_base_farmer_reward, calculate_pool_reward
...
from wheat.data_layer.dl_wallet_store import DataLayerStore
from wheat.pools.pool_puzzles import (
...
from wheat.util.db_wrapper import DBWrapper2
from wheat.util.errors import Err
from wheat.util.hash import std_hash
from wheat.util.ints import uint16, uint32, uint64, uint128
from wheat.util.lru_cache import LRUCache
from wheat.util.misc import UInt32Range, UInt64Range, VersionedBlob
...
from wheat.wallet.cat_wallet.cat_info import CATCoinData, CATInfo, CRCATInfo
from wheat.wallet.cat_wallet.cat_utils import CAT_MOD, CAT_MOD_HASH, construct_cat_puzzle, match_cat_puzzle
from wheat.wallet.cat_wallet.cat_wallet import CATWallet
from wheat.wallet.cat_wallet.dao_cat_wallet import DAOCATWallet
from wheat.wallet.conditions import Condition, ConditionValidTimes, parse_timelock_info
...
from wheat.wallet.db_wallet.db_wallet_puzzles import MIRROR_PUZZLE_HASH
...
from wheat.wallet.did_wallet.did_info import DIDCoinData
from wheat.wallet.did_wallet.did_wallet import DIDWallet
from wheat.wallet.did_wallet.did_wallet_puzzles import DID_INNERPUZ_MOD, match_did_puzzle
from wheat.wallet.key_val_store import KeyValStore
from wheat.wallet.nft_wallet.nft_puzzles import get_metadata_and_phs, get_new_owner_did
from wheat.wallet.nft_wallet.nft_wallet import NFTWallet
from wheat.wallet.nft_wallet.uncurry_nft import NFTCoinData, UncurriedNFT
from wheat.wallet.notification_manager import NotificationManager
from wheat.wallet.outer_puzzles import AssetType
from wheat.wallet.payment import Payment
from wheat.wallet.puzzle_drivers import PuzzleInfo
from wheat.wallet.puzzles.clawback.drivers import generate_clawback_spend_bundle, match_clawback_puzzle
from wheat.wallet.puzzles.clawback.metadata import ClawbackMetadata, ClawbackVersion
from wheat.wallet.puzzles.p2_delegated_puzzle_or_hidden_puzzle import (
    DEFAULT_HIDDEN_PUZZLE_HASH,
    calculate_synthetic_secret_key,
    puzzle_hash_for_synthetic_public_key,
)
from wheat.wallet.sign_coin_spends import sign_coin_spends
from wheat.wallet.singleton import create_singleton_puzzle, get_inner_puzzle_from_singleton, get_singleton_id_from_puzzle
from wheat.wallet.trade_manager import TradeManager
...
from wheat.wallet.util.address_type import AddressType
...
from wheat.wallet.util.puzzle_decorator import PuzzleDecoratorManager
from wheat.wallet.util.query_filter import HashFilter
from wheat.wallet.util.transaction_type import CLAWBACK_INCOMING_TRANSACTION_TYPES, TransactionType
from wheat.wallet.util.tx_config import TXConfig, TXConfigLoader
from wheat.wallet.util.wallet_sync_utils import (
    PeerRequestException,
    fetch_coin_spend_for_coin_state,
    last_change_height_cs,
)
from wheat.wallet.util.wallet_types import CoinType, WalletIdentifier, WalletType
from wheat.wallet.vc_wallet.cr_cat_drivers import CRCAT, ProofsChecker, construct_pending_approval_state
from wheat.wallet.vc_wallet.cr_cat_wallet import CRCATWallet
from wheat.wallet.vc_wallet.vc_drivers import VerifiedCredential
from wheat.wallet.vc_wallet.vc_store import VCStore
from wheat.wallet.vc_wallet.vc_wallet import VCWallet
from wheat.wallet.wallet import Wallet
from wheat.wallet.wallet_blockchain import WalletBlockchain
from wheat.wallet.wallet_coin_record import MetadataTypes, WalletCoinRecord
from wheat.wallet.wallet_coin_store import WalletCoinStore
from wheat.wallet.wallet_info import WalletInfo
from wheat.wallet.wallet_interested_store import WalletInterestedStore
from wheat.wallet.wallet_nft_store import WalletNftStore
from wheat.wallet.wallet_pool_store import WalletPoolStore
from wheat.wallet.wallet_protocol import WalletProtocol
from wheat.wallet.wallet_puzzle_store import WalletPuzzleStore
from wheat.wallet.wallet_retry_store import WalletRetryStore
from wheat.wallet.wallet_transaction_store import WalletTransactionStore
from wheat.wallet.wallet_user_store import WalletUserStore
...
class WalletStateManager:
    ...
    async def determine_coin_type(
        self, peer: WSWheatConnection, coin_state: CoinState, fork_height: Optional[uint32]
    ...
    async def spend_clawback_coins(
        ...
        for coin, metadata in clawback_coins.items():
            try:
                ...
                # Remove the clawback hint since it is unnecessary for the WHEAT coin
        ...
        if fee > 0:
            wheat_tx = await self.main_wallet.create_tandem_wheat_tx(
                fee, tx_config, Announcement(coin_spends[0].coin.name(), message)
            )
            assert wheat_tx.spend_bundle is not None
            spend_bundle = SpendBundle.aggregate([spend_bundle, wheat_tx.spend_bundle])
        assert derivation_record is not None
    ...
    async def filter_spam(self, new_coin_state: List[CoinState]) -> List[CoinState]:
        wheat_spam_amount = self.config.get("wheat_spam_amount", 1000000)

        # No need to filter anything if the filter is set to 1 or 0 mojos
        if wheat_spam_amount <= 1:
            return new_coin_state

        spam_filter_after_n_txs = self.config.get("spam_filter_after_n_txs", 200)
        small_unspent_count = await self.coin_store.count_small_unspent(wheat_spam_amount)
        ...
        for cs in new_coin_state:
            ...
            if (
                ...
                and cs.coin.amount < wheat_spam_amount
    ...
    async def get_minter_did(self, launcher_coin: Coin, peer: WSWheatConnection) -> Optional[bytes32]:
        # Get minter DID
