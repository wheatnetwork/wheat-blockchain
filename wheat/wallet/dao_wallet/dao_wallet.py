...
from wheat.protocols.wallet_protocol import CoinState, RequestBlockHeader, RespondBlockHeader
...
from wheat.wallet import singleton
from wheat.wallet.cat_wallet.cat_utils import CAT_MOD, SpendableCAT, construct_cat_puzzle
from wheat.wallet.cat_wallet.cat_utils import get_innerpuzzle_from_puzzle as get_innerpuzzle_from_cat_puzzle
from wheat.wallet.cat_wallet.cat_utils import unsigned_spend_bundle_for_spendable_cats
...
from wheat.wallet.conditions import Condition, parse_timelock_info
from wheat.wallet.dao_wallet.dao_info import DAOInfo, DAORules, ProposalInfo, ProposalType
...
from wheat.wallet.util.tx_config import DEFAULT_TX_CONFIG, CoinSelectionConfig, TXConfig
...
class DAOWallet:
    ...
    It is not a user-facing wallet. A user cannot spend or receive WHEAT though a wallet of this type.
    ...
    # if asset_id == None: then we get normal WHEAT
    ...
    async def coin_added(self, coin: Coin, height: uint32, peer: WSWheatConnection, coin_data: Optional[Any]) -> None:
        """
        ...
        try:
            ...
            if matched_funding_puz:
                # funding coin
                wheat_funds_puzhash = get_p2_singleton_puzhash(self.dao_info.treasury_id, asset_id=None)
                if coin.puzzle_hash == wheat_funds_puzhash:
                    asset_id = None
                else:
    ...
    async def resync_treasury_state(self) -> None:
        ...
        wallet_node: Any = self.wallet_state_manager.wallet_node
        peer: WSWheatConnection = wallet_node.get_full_node_peer()
        if peer is None:  # pragma: no cover
            raise ValueError("Could not find any peers to request puzzle and solution from")
        ...
        parent_inner_puz = wheat.wallet.singleton.get_inner_puzzle_from_singleton(
        ...
        # get existing wheat funds for treasury
        wheat_funds_puzhash = get_p2_singleton_puzhash(self.dao_info.treasury_id, asset_id=None)
        await self.wallet_state_manager.add_interested_puzzle_hashes([wheat_funds_puzhash], [self.id()])
    ...
    async def generate_proposal_vote_spend(
        ...
        if fee > 0:
            ...
            spend_bundle = unsigned_spend_bundle.aggregate([unsigned_spend_bundle, dao_cat_spend, wheat_tx.spend_bundle])
        else:
    ...
    async def create_proposal_close_spend(
        ...
        if proposal_state["passed"] and not self_destruct:
            ...
            if proposal_type == ProposalType.SPEND:
                ...
                if sum > 0:
                    wheat_coins = await self.select_coins_for_asset_type(uint64(sum))
                    for wheat_coin in wheat_coins:
                        wheat_parent_amount_list.append([wheat_coin.parent_coin_info, wheat_coin.amount])
                        solution = Program.to(
                            [
                                0,
                                treasury_inner_puzhash,
                        ...
                        coin_spends.append(CoinSpend(wheat_coin, p2_singleton_puzzle, solution))
        ...
        if fee > 0:
            wheat_tx = await self.standard_wallet.create_tandem_wheat_tx(fee, tx_config)
            assert wheat_tx.spend_bundle is not None
            full_spend = SpendBundle.aggregate([spend_bundle, wheat_tx.spend_bundle])
        else:
    ...
    async def free_coins_from_finished_proposals(
        ...
        if fee > 0:
            wheat_tx = await self.standard_wallet.create_tandem_wheat_tx(fee, tx_config)
            assert wheat_tx.spend_bundle is not None
            full_spend = full_spend.aggregate([full_spend, wheat_tx.spend_bundle])
            ...
            if prop_info.proposal_id == proposal_id:
                ...
                if proposal_type == ProposalType.SPEND:
                    ...
                    dictionary: Dict[str, Any] = {
                        ...
                        "wheat_conditions": wheat_created_coins,
