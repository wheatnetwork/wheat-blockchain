...
from wheat.util.bech32m import bech32_encode, bech32_decode, convertbits
...
class Offer:
    ...
    def get_pending_amounts(self) -> Dict[str, int]:
        ...
        # Then we add a potential fee as pending WHEAT
        fee: int = sum(c.amount for c in all_removals) - sum(c.amount for c in all_additions)
        if fee > 0:
            pending_dict.setdefault("wheat", 0)
            pending_dict["wheat"] += fee
