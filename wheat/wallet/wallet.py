...
from wheat.wallet.secret_key_store import SecretKeyStore
...
CHIP_0002_SIGN_MESSAGE_PREFIX = "Wheat Signed Message"
...
class Wallet:
    ...
    async def create_tandem_wheat_tx(
        ...
        wheat_coins = await self.select_coins(fee)
        if reuse_puzhash is None:
            reuse_puzhash_config = self.wallet_state_manager.config.get("reuse_public_key_for_change", None)
            if reuse_puzhash_config is None:
                reuse_puzhash = False
    ...
    async def coin_added(
        self, coin: Coin, height: uint32, peer: WSWheatConnection
