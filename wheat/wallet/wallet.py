...
from wheat.consensus.cost_calculator import calculate_cost_of_program, NPCResult
...
from wheat.wallet.sign_coin_solutions import sign_coin_solutions
...
class Wallet:
    ...
    async def create_spend_bundle_relative_wheat(self, wheat_amount: int, exclude: List[Coin]) -> SpendBundle:
        ...
        if wheat_amount < 0:
            utxos = await self.select_coins(abs(wheat_amount), exclude)
        else:
        ...
        for coin in utxos:
            ...
            if output_created is None:
                ...
                primaries = [{"puzzlehash": newpuzhash, "amount": wheat_amount}]
