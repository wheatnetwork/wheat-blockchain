...
class Wallet:
    ...
    async def create_spend_bundle_relative_wheat(self, wheat_amount: int, exclude: List[Coin] = []) -> SpendBundle:
        list_of_solutions = []
        utxos = None
        ...
        if wheat_amount < 0:
            utxos = await self.select_coins(uint64(abs(wheat_amount)), exclude)
        else:
        ...
        wheat_amount = spend_value + wheat_amount
        ...
        for coin in utxos:
            ...
            if output_created is None:
                ...
                primaries: List[AmountWithPuzzlehash] = [
                    {"puzzlehash": newpuzhash, "amount": uint64(wheat_amount), "memos": []}
                ]
