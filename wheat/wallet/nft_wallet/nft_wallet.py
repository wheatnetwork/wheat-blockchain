...
from wheat.wallet.conditions import Condition, UnknownCondition, parse_timelock_info
...
from wheat.wallet.nft_wallet.nft_info import NFTCoinInfo, NFTWalletInfo
from wheat.wallet.nft_wallet.nft_puzzles import NFT_METADATA_UPDATER, create_ownership_layer_puzzle, get_metadata_and_phs
from wheat.wallet.nft_wallet.uncurry_nft import NFTCoinData, UncurriedNFT
from wheat.wallet.outer_puzzles import AssetType, construct_puzzle, match_puzzle, solve_puzzle
...
from wheat.wallet.trading.offer import OFFER_MOD, OFFER_MOD_HASH, NotarizedPayment, Offer
...
from wheat.wallet.util.tx_config import CoinSelectionConfig, TXConfig
from wheat.wallet.util.wallet_types import WalletType
...
from wheat.wallet.wallet_protocol import GSTOptionalArgs, WalletProtocol
...
class NFTWallet:
    ...
    async def coin_added(
        self, coin: Coin, height: uint32, peer: WSWheatConnection, parent_coin_data: Optional[NFTCoinData]
    ) -> None:
    ...
    async def puzzle_solution_received(self, coin: Coin, data: NFTCoinData, peer: WSWheatConnection) -> None:
    ...
    async def generate_signed_transaction(
        ...
        **kwargs: Unpack[GSTOptionalArgs],
        ...
        unsigned_spend_bundle, wheat_tx = await self.generate_unsigned_spendbundle(
            payments,
            tx_config,
            fee,
            coins=coins,
        ...
        if wheat_tx is not None and wheat_tx.spend_bundle is not None:
            spend_bundle = SpendBundle.aggregate([spend_bundle, wheat_tx.spend_bundle])
            wheat_tx = dataclasses.replace(wheat_tx, spend_bundle=None)
        ...
        if wheat_tx is not None:
            tx_list.append(wheat_tx)
    ...
    async def generate_unsigned_spendbundle(
        ...
        if fee > 0:
            ...
            wheat_tx = await self.standard_wallet.create_tandem_wheat_tx(
        ...
        return nft_spend_bundle, wheat_tx
        ...
        for asset, amount in offer_dict.items():
            if amount < 0:
                ...
                if len(offered_coins) == 0:
                    raise ValueError(f"Did not have asset ID {asset.hex() if asset is not None else 'WHEAT'} to offer")
    ...
    async def mint_from_did(
        ...
        wheat_coins: Optional[Set[Coin]] = None,
        wheat_change_ph: Optional[bytes32] = None,
        ...
        - The launcher coins are then spent along with the created eve spend
          and an wheat spend that funds the transactions and pays fees.
        ...
        :param wheat_coins: [Optional] For use with bulk minting to provide the coin used for funding the minting spend.
        This coin can be one that will be created in the future
        :param wheat_change_ph: [Optional] For use with bulk minting, so we can specify the puzzle hash that the change
        ...
        :param fee: A fee amount, taken out of the wheat spend.
        """
        # get DID Wallet
        ...
        # Ensure we have an wheat coin of high enough amount
        assert isinstance(fee, uint64)
        total_amount = len(metadata_list) + fee
        if wheat_coins is None:
            wheat_coins = await self.standard_wallet.select_coins(uint64(total_amount), tx_config.coin_selection_config)
        assert len(wheat_coins) > 0
        ...
        for mint_number in range(mint_number_start, mint_number_end):
            ...
            intermediate_launcher_puz = did_wallet_puzzles.INTERMEDIATE_LAUNCHER_MOD.curry(
                wheat.wallet.singleton.SINGLETON_LAUNCHER_PUZZLE_HASH, mint_number, mint_total
            )
            ...
            launcher_coin = Coin(
                intermediate_launcher_coin.name(), wheat.wallet.singleton.SINGLETON_LAUNCHER_PUZZLE_HASH, amount
            )
            ...
            launcher_cs = CoinSpend(
                launcher_coin, wheat.wallet.singleton.SINGLETON_LAUNCHER_PUZZLE, genesis_launcher_solution
            )
        ...
        # Create the wheat spend to fund the minting.
        spend_value = sum([coin.amount for coin in wheat_coins])
        change: uint64 = uint64(spend_value - total_amount)
        wheat_spends = []
        if wheat_change_ph is None:
            wheat_change_ph = await self.standard_wallet.get_new_puzzlehash()
        wheat_payment = Payment(wheat_change_ph, change, [wheat_change_ph])

        first = True
        for wheat_coin in wheat_coins:
            puzzle: Program = await self.standard_wallet.puzzle_for_puzzle_hash(wheat_coin.puzzle_hash)
            if first:
                message_list: List[bytes32] = [c.name() for c in wheat_coins]
                message_list.append(Coin(wheat_coin.name(), wheat_payment.puzzle_hash, wheat_payment.amount).name())
                message: bytes32 = std_hash(b"".join(message_list))

                if len(wheat_coins) > 1:
                    wheat_announcement: Optional[Set[bytes]] = {message}
                else:
                    wheat_announcement = None

                solution: Program = self.standard_wallet.make_solution(
                    primaries=[wheat_payment],
                    fee=fee,
                    coin_announcements=wheat_announcement,
                    coin_announcements_to_assert={Announcement(did_coin.name(), message).name()},
                )
                primary_announcement_hash = Announcement(wheat_coin.name(), message).name()
            ...
            wheat_spends.append(CoinSpend(wheat_coin, puzzle, solution))
        wheat_spend = await self.wallet_state_manager.sign_transaction(wheat_spends)
        ...
        did_full_puzzle: Program = wheat.wallet.singleton.create_singleton_puzzle(
            innerpuz,
        ...
        total_spend = SpendBundle.aggregate([signed_spend_bundle, wheat_spend, *eve_spends])

        tx_record: TransactionRecord = dataclasses.replace(eve_txs[0], spend_bundle=total_spend)
        return [tx_record]

    async def mint_from_wheat(
        self,
        metadata_list: List[Dict[str, Any]],
        tx_config: TXConfig,
        target_list: Optional[List[bytes32]] = [],
        ...
        """
        Minting NFTs from a single WHEAT spend using intermediate launcher puzzle
        :param metadata_list: A list of dicts containing the metadata for each NFT to be minted
        :param target_list: [Optional] a list of targets for transfering minted NFTs (aka airdrop)
        :param mint_number_start: [Optional] The starting point for mint number used in intermediate launcher
        puzzle. Default: 1
        ...
        funding_coin = wheat_coins.copy().pop()
        ...
        # Empty set to load with the announcements we will assert from WHEAT to
        # match the announcements from the intermediate launcher puzzle
        ...
        for mint_number in range(mint_number_start, mint_number_end):
            ...
            # create an ASSERT_COIN_ANNOUNCEMENT for the WHEAT spend. The
            # intermediate launcher coin issues a CREATE_COIN_ANNOUNCEMENT of
            # the mint_number and mint_total for the launcher coin it creates
            intermediate_announcement_message = std_hash(int_to_bytes(mint_number) + int_to_bytes(mint_total))
            coin_announcements.add(std_hash(intermediate_launcher_coin.name() + intermediate_announcement_message))

            # Create the launcher coin, and add its id to a list to be asserted in the WHEAT spend
            ...
            # Annnouncements for eve spend. These are asserted by the wheat spend
            announcement_message = Program.to([eve_fullpuz.get_tree_hash(), amount, []]).get_tree_hash()
        ...
        for wheat_coin in wheat_coins:
            ...
            if first:
                ...
                solution: Program = self.standard_wallet.make_solution(
                    primaries=[wheat_payment] + primaries,
                    fee=fee,
                    coin_announcements=wheat_announcement if len(wheat_coins) > 1 else None,
