...
from wheat.wallet.trading.offer import OFFER_MOD, NotarizedPayment, Offer
from wheat.wallet.transaction_record import TransactionRecord
from wheat.wallet.util.compute_memos import compute_memos
from wheat.wallet.util.debug_spend_bundle import disassemble
from wheat.wallet.util.transaction_type import TransactionType
from wheat.wallet.util.wallet_types import AmountWithPuzzlehash, WalletType
...
class NFTWallet:
    ...
    async def coin_added(self, coin: Coin, height: uint32, in_transaction: bool) -> None:
        ...
        full_nodes: Dict[bytes32, WSWheatConnection] = server.connection_by_type.get(NodeType.FULL_NODE, {})
    ...
    async def create_tandem_wheat_tx(
        ...
        wheat_coins = await self.standard_wallet.select_coins(fee)
    ...
    async def generate_unsigned_spendbundle(
        ...
        if fee > 0:
            ...
            wheat_tx = await self.create_tandem_wheat_tx(fee, Announcement(nft_coin.coin.name(), announcement_to_make))
        else:
