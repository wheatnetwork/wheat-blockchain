...
from wheat.wallet.trading.offer import (
    OFFER_MOD,
    OFFER_MOD_HASH,
    OFFER_MOD_OLD,
    OFFER_MOD_OLD_HASH,
...
from wheat.wallet.util.wallet_sync_utils import fetch_coin_spend
...
class NFTWallet:
    ...
    async def coin_added(self, coin: Coin, height: uint32, peer: WSWheatConnection) -> None:
    ...
    async def puzzle_solution_received(self, coin_spend: CoinSpend, peer: WSWheatConnection) -> None:
    ...
    async def mint_from_did(
        ...
        if wheat_coins is None:
            wheat_coins = await self.standard_wallet.select_coins(uint64(total_amount))
        ...
        wheat_spend = await self.standard_wallet.sign_transaction(wheat_spends)
