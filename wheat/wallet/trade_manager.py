...
from wheat.wallet.conditions import Condition, ConditionValidTimes, parse_conditions_non_consensus, parse_timelock_info
from wheat.wallet.db_wallet.db_wallet_puzzles import ACS_MU_PH
...
from wheat.wallet.trading.trade_store import TradeStore
...
from wheat.wallet.vc_wallet.cr_cat_drivers import ProofsChecker, construct_pending_approval_state
...
class TradeManager:
    ...
    By default, standard WHEAT is supported but to support other types of assets you must implement certain functions on
    ...
    Puzzle Drivers:
      - See wheat/wallet/outer_puzzles.py for a full description of how to build these
    ...
    async def coins_of_interest_farmed(
        self, coin_state: CoinState, fork_height: Optional[uint32], peer: WSWheatConnection
    ) -> None:
        """
    ...
    async def cancel_pending_offers(
        ...
        for trade_id in trades:
            ...
            for coin in Offer.from_bytes(trade.offer).get_cancellation_coins():
                ...
                # This should probably not switch on whether or not we're spending a WHEAT but it has to for now
    ...
    async def _create_offer_for_ids(
        ...
        try:
            ...
            for id, amount in offer_dict.items():
                ...
                # asset_id can either be none if asset is WHEAT or
                ...
                if amount > 0:
                    ...
                    # For the WHEAT wallet also include the fee amount to the coins we use to pay this offer
                ...
                if asset_id is not None and wallet is not None:  # if this asset is not WHEAT
            ...
            # The access of the sorted keys here makes sure we create the WHEAT transaction first to make sure we pay fee
            # with the WHEAT side of the offer and don't create an extra fee transaction in other wallets.
            for id in sorted(coins_to_offer.keys()):
                ...
                # This should probably not switch on whether or not we're spending WHEAT but it has to for now
                if wallet.type() == WalletType.STANDARD_WALLET:
                    [tx] = await wallet.generate_signed_transaction(
                        abs(offer_dict[id]),
                        Offer.ph(),
    ...
    async def check_offer_validity(self, offer: Offer, peer: WSWheatConnection) -> bool:
