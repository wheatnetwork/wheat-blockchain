...
from wheat.types.coin_spend import compute_additions_with_cost
...
from wheat.wallet.cat_wallet.cat_info import CATCoinData, CATInfo, LegacyCATInfo
...
from wheat.wallet.coin_selection import select_coins
from wheat.wallet.conditions import Condition, ConditionValidTimes, UnknownCondition, parse_timelock_info
...
from wheat.wallet.puzzles.tails import ALL_LIMITATIONS_PROGRAMS
...
from wheat.wallet.util.curry_and_treehash import calculate_hash_of_quoted_mod_hash, curry_and_treehash
...
class CATWallet:
    ...
    async def create_new_cat_wallet(
        ...
        try:
            wheat_tx, spend_bundle = await ALL_LIMITATIONS_PROGRAMS[
        ...
        wheat_tx = dataclasses.replace(wheat_tx, spend_bundle=spend_bundle, name=spend_bundle.name())
        await self.standard_wallet.push_transaction(wheat_tx)
    ...
    async def coin_added(
        self, coin: Coin, height: uint32, peer: WSWheatConnection, parent_coin_data: Optional[CATCoinData]
    ) -> None:
    ...
    async def create_tandem_wheat_tx(
        self,
        fee: uint64,
        ...
        if fee > amount_to_claim:
            wheat_coins = await self.standard_wallet.select_coins(
                fee,
                tx_config.coin_selection_config,
            )
            origin_id = list(wheat_coins)[0].name()
            [wheat_tx] = await self.standard_wallet.generate_signed_transaction(
                uint64(0),
            ...
            origin_id = list(wheat_coins)[0].name()
            selected_amount = sum([c.amount for c in wheat_coins])
            [wheat_tx] = await self.standard_wallet.generate_signed_transaction(
            ...
            message = None
            for spend in wheat_tx.spend_bundle.coin_spends:
    ...
    async def generate_unsigned_spendbundle(
        ...
        # Figure out if we need to absorb/melt some WHEAT as part of this
        regular_wheat_to_claim: int = 0
        if payment_amount > starting_amount:
            fee = uint64(fee + payment_amount - starting_amount)
        elif payment_amount < starting_amount:
            regular_wheat_to_claim = payment_amount

        need_wheat_transaction = (fee > 0 or regular_wheat_to_claim > 0) and (fee - regular_wheat_to_claim != 0)
        ...
        for coin in cat_coins:
            ...
            if first:
                ...
                if need_wheat_transaction:
                    if fee > regular_wheat_to_claim:
                        wheat_tx, _ = await self.create_tandem_wheat_tx(
                            fee,
                            uint64(regular_wheat_to_claim),
                            tx_config,
                    ...
                    elif regular_wheat_to_claim > fee:
                        wheat_tx, _ = await self.create_tandem_wheat_tx(
                            fee,
                            uint64(regular_wheat_to_claim),
                            tx_config,
        ...
        wheat_spend_bundle = SpendBundle([], G2Element())
        if wheat_tx is not None and wheat_tx.spend_bundle is not None:
            wheat_spend_bundle = wheat_tx.spend_bundle
    ...
    async def generate_signed_transaction(
        ...
        **kwargs: Unpack[GSTOptionalArgs],
        ...
        if wheat_tx is not None:
            tx_list.append(
                TransactionRecord(
                    confirmed_at_height=wheat_tx.confirmed_at_height,
                    created_at_time=wheat_tx.created_at_time,
                    to_puzzle_hash=wheat_tx.to_puzzle_hash,
                    amount=wheat_tx.amount,
                    fee_amount=wheat_tx.fee_amount,
                    confirmed=wheat_tx.confirmed,
                    sent=wheat_tx.sent,
                    spend_bundle=None,
                    additions=wheat_tx.additions,
                    removals=wheat_tx.removals,
                    wallet_id=wheat_tx.wallet_id,
                    sent_to=wheat_tx.sent_to,
                    trade_id=wheat_tx.trade_id,
                    type=wheat_tx.type,
                    name=wheat_tx.name,
                    memos=[],
                    valid_times=parse_timelock_info(extra_conditions),
