...
from wheat.consensus.blockchain import ReceiveBlockResult
...
from wheat.types.weight_proof import WeightProof, SubEpochData
from wheat.util.byte_types import hexstr_to_bytes
from wheat.util.config import WALLET_PEERS_PATH_KEY_DEPRECATED
from wheat.util.default_root import STANDALONE_ROOT_PATH
from wheat.util.ints import uint32, uint64
from wheat.util.keychain import KeyringIsLocked, Keychain
from wheat.util.path import mkdir, path_from_root
from wheat.wallet.util.new_peak_queue import NewPeakQueue, NewPeakQueueTypes, NewPeakItem
...
class WalletNode:
    ...
    def initialize_wallet_peers(self):
        ...
        if self.wallet_peers is None and connect_to_unknown_peers and not testing:
            self.wallet_peers = WalletPeers(
                ...
                self.config.get("dns_servers", ["dns-introducer.wheat.network"]),
    ...
    def on_disconnect(self, peer: WSWheatConnection):
    ...
    async def on_connect(self, peer: WSWheatConnection):
        if self.wallet_state_manager is None:
            return None
    ...
    async def is_peer_synced(
        self, peer: WSWheatConnection, header_block: HeaderBlock, request_time: uint64
    ) -> Optional[uint64]:
        # Get last timestamp
    ...
    async def state_update_received(self, request: wallet_protocol.CoinStateUpdate, peer: WSWheatConnection) -> None:
    ...
    def get_full_node_peer(self) -> Optional[WSWheatConnection]:
        if self.server is None:
            return None
    ...
    async def get_timestamp_for_height(self, height: uint32) -> uint64:
        ...
        peer: Optional[WSWheatConnection] = self.get_full_node_peer()
        if peer is None:
    ...
    async def new_peak_wallet(self, new_peak: wallet_protocol.NewPeakWallet, peer: WSWheatConnection):
        if self.wallet_state_manager is None:
    ...
    async def fetch_and_validate_the_weight_proof(
        self, peer: WSWheatConnection, peak: HeaderBlock
    ...
    async def fetch_puzzle_solution(self, peer: WSWheatConnection, height: uint32, coin: Coin) -> CoinSpend:
    ...
    async def get_coin_state(
        self, coin_names: List[bytes32], fork_height: Optional[uint32] = None, peer: Optional[WSWheatConnection] = None
    ) -> List[CoinState]:
        assert self.server is not None
        all_nodes = self.server.connection_by_type[NodeType.FULL_NODE]
        if len(all_nodes.keys()) == 0:
    ...
    async def fetch_children(
        self, peer: WSWheatConnection, coin_name: bytes32, fork_height: Optional[uint32] = None
    ) -> List[CoinState]:
