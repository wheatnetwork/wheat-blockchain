...
from wheat.wallet.vc_wallet.vc_store import VCRecord, VCStore
...
class VCWallet:
    ...
    async def revoke_vc(
        self, parent_id: bytes32, peer: WSWheatConnection, fee: uint64 = uint64(0), reuse_puzhash: Optional[bool] = None
    ) -> List[TransactionRecord]:
        ...
        if fee > 0:
            ...
            tx = dataclasses.replace(tx, spend_bundle=SpendBundle.aggregate([wheat_tx.spend_bundle, tx.spend_bundle]))
...
if TYPE_CHECKING:
    from wheat.wallet.wallet_protocol import WalletProtocol  # pragma: no cover
