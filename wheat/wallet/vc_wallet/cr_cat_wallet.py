...
from wheat.util.misc import VersionedBlob
from wheat.wallet.cat_wallet.cat_info import CATCoinData, CRCATInfo
from wheat.wallet.cat_wallet.cat_utils import CAT_MOD_HASH, CAT_MOD_HASH_HASH, construct_cat_puzzle
...
from wheat.wallet.util.tx_config import TXConfig
...
from wheat.wallet.vc_wallet.cr_cat_drivers import (
    CRCAT,
    CRCATMetadata,
    CRCATVersion,
    ProofsChecker,
...
class CRCATWallet(CATWallet):
    ...
    async def coin_added(
        self, coin: Coin, height: uint32, peer: WSWheatConnection, coin_data: Optional[CATCoinData]
    ) -> None:
    ...
    async def _generate_unsigned_spendbundle(
            ...
            if first:
                ...
                if need_wheat_transaction:
                    if fee > regular_wheat_to_claim:
                        ...
                        wheat_tx, wheat_announcement = await self.create_tandem_wheat_tx(
                            fee,
                            uint64(regular_wheat_to_claim),
                            tx_config,
                        )
                        assert wheat_announcement is not None
                        innersol = self.standard_wallet.make_solution(
                            primaries=primaries,
                            coin_announcements={announcement.message},
                            coin_announcements_to_assert={wheat_announcement.name()},
                            conditions=extra_conditions,
                        )
                else:
                    innersol = self.standard_wallet.make_solution(
        ...
        return (
            SpendBundle(
                [
                    ...
                    *(
                        (
                            spend
                            for bundle in [wheat_tx.spend_bundle]
            ...
            [*vc_txs, *([wheat_tx] if wheat_tx is not None else [])],
        )
    ...
    async def claim_pending_approval_balance(
        ...
        # Select the relevant WHEAT coins
        if fee > 0:
        ...
        nonce: bytes32 = Program.to(
            [coin_as_list(c) for c in sorted(coins.union(wheat_coins).union({vc.coin}), key=Coin.name)]
        ).get_tree_hash()
        ...
        if fee > 0:
            ...
            if wheat_tx.spend_bundle is None:
                raise RuntimeError("Did not get spendbundle for fee transaction")  # pragma: no cover
            claim_bundle = SpendBundle.aggregate([claim_bundle, wheat_tx.spend_bundle])
        else:
            wheat_tx = None
        ...
        other_txs: List[TransactionRecord] = [
            ...
            *((dataclasses.replace(wheat_tx, spend_bundle=None),) if wheat_tx is not None else []),
        ]
