...
from wheat.plot_sync.delta import Delta
from wheat.plot_sync.receiver import Receiver
from wheat.pools.pool_config import PoolWalletConfig, add_auth_key, load_pool_config
...
from wheat.server.server import WheatServer, ssl_context_for_root
...
from wheat.util.config import config_path_for_filename, load_config, lock_and_load_config, save_config
from wheat.util.errors import KeychainProxyConnectionFailure
...
from wheat.util.logging import TimedDuplicateFilter
...
from wheat.wallet.puzzles.singleton_top_layer import SINGLETON_MOD
...
class Farmer:
    ...
    async def setup_keys(self) -> bool:
        no_keys_error_str = "No keys exist. Please run 'wheat keys generate' or open the UI."
        try:
        ...
        if "wheat_target_address" not in self.config:
            self.config = config["farmer"]
        if "wheat_target_address" not in self.pool_config:
            self.pool_config = config["pool"]
        if "wheat_target_address" not in self.config or "wheat_target_address" not in self.pool_config:
            log.debug("wheat_target_address missing in the config")
            return False

        # This is the farmer configuration
        self.farmer_target_encoded = self.config["wheat_target_address"]
    ...
    def set_reward_targets(self, farmer_target_encoded: Optional[str], pool_target_encoded: Optional[str]) -> None:
        with lock_and_load_config(self._root_path, "config.yaml") as config:
            if farmer_target_encoded is not None:
                ...
                config["farmer"]["wheat_target_address"] = farmer_target_encoded
            if pool_target_encoded is not None:
                self.pool_target_encoded = pool_target_encoded
                self.pool_target = decode_puzzle_hash(pool_target_encoded)
                config["pool"]["wheat_target_address"] = pool_target_encoded
