...
from wheat.consensus.blockchain import AddBlockResult, Blockchain, StateChangeSummary
...
from wheat.full_node.lock_queue import LockClient, LockQueue
...
from wheat.full_node.sync_store import SyncStore
...
from wheat.types.blockchain_format.vdf import CompressibleVDFField, VDFInfo, VDFProof
...
from wheat.util.errors import ConsensusError, Err, ValidationError
...
class FullNode:
    ...
    def set_server(self, server: WheatServer) -> None:
        ...
        if "dns_servers" in self.config:
            ...
            dns_servers.append("dns-introducer.wheatcoin.top")
        try:
    ...
    async def _sync(self) -> None:
        ...
        try:
            ...
            peers_with_peak: List[WSWheatConnection] = [
    ...
    async def update_wallets(
        ...
        for peer, changes in changes_for_peer.items():
            if peer not in self.server.all_connections:
                continue
            ws_peer: WSWheatConnection = self.server.all_connections[peer]
