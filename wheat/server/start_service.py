...
from wheat.cmds.init_funcs import wheat_full_version_str
from wheat.daemon.server import service_launch_lock_path
from wheat.rpc.rpc_server import RpcApiProtocol, RpcServer, RpcServiceProtocol, start_rpc_server
from wheat.server.api_protocol import ApiProtocol
from wheat.server.wheat_policy import set_wheat_policy
from wheat.server.outbound_message import NodeType
from wheat.server.server import WheatServer
from wheat.server.ssl_context import wheat_ssl_ca_paths, private_ssl_ca_paths
from wheat.server.upnp import UPnP
from wheat.server.ws_connection import WSWheatConnection
from wheat.types.peer_info import PeerInfo, UnresolvedPeerInfo
...
class Service(Generic[_T_RpcServiceProtocol, _T_ApiProtocol]):
    def __init__(
        ...
        *,
        ...
        on_connect_callback: Optional[Callable[[WSWheatConnection], Awaitable[None]]] = None,
        ...
        self._log.info(f"wheat-blockchain version: {wheat_full_version_str()}")

        self.service_config = self.config[service_name]

        self._rpc_info = rpc_info
        private_ca_crt, private_ca_key = private_ssl_ca_paths(root_path, self.config)
        wheat_ca_crt, wheat_ca_key = wheat_ssl_ca_paths(root_path, self.config)
        ...
        self._server = WheatServer.create(
            advertised_port,
            node,
            peer_api,
            node_type,
    ...
    async def manage(self, *, start: bool = True) -> AsyncIterator[None]:
        ...
        async with contextlib.AsyncExitStack() as async_exit_stack:
            try:
                ...
                self._log.info("Waiting for WheatServer to be closed")
    ...
    async def setup_process_global_state(self, signal_handlers: SignalHandlers) -> None:
        ...
        proctitle_name = f"wheat_{self._service_name}"
...
def async_run(coro: Coroutine[object, object, T], connection_limit: Optional[int] = None) -> T:
    with log_exceptions(log=log, message="fatal uncaught exception"):
        if connection_limit is not None:
            set_wheat_policy(connection_limit)
