...
from wheat.server.ws_connection import ConnectionCallback, WSWheatConnection
...
class WheatServer:
    ...
    all_connections: Dict[bytes32, WSWheatConnection] = field(default_factory=dict)
    ...
    def create(
        ...
        wheat_ca_crt_path, wheat_ca_key_path = wheat_ca_crt_key
        ...
        if local_type in authenticated_client_types:
            ...
            ssl_client_context = ssl_context_for_client(
                ca_cert=wheat_ca_crt_path,
                ca_key=wheat_ca_key_path,
    ...
    async def start(
        ...
        if self.webserver is not None:
            raise RuntimeError("WheatServer already started")
    ...
    async def connection_added(
        self, connection: WSWheatConnection, on_connect: Optional[ConnectionCallback] = None
    ) -> None:
    ...
    def connection_closed(self, connection: WSWheatConnection, ban_time: int, closed_connection: bool = False) -> None:
