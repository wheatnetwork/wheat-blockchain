...
from wheat.protocols.protocol_timing import API_EXCEPTION_BAN_SECONDS, INVALID_PROTOCOL_BAN_SECONDS
...
from wheat.util.errors import Err, ProtocolError
from wheat.util.ints import uint16
from wheat.util.network import is_in_network, is_localhost
...
class WheatServer:
    def __init__(
        ...
        wheat_ca_crt_key: Tuple[Path, Path],
        name: str = None,
        introducer_peers: Optional[IntroducerPeers] = None,
    ):
        # Keeps track of all connections to and from this node.
        logging.basicConfig(level=logging.DEBUG)
        self.all_connections: Dict[bytes32, WSWheatConnection] = {}
        self.tasks: Set[asyncio.Task] = set()

        self.connection_by_type: Dict[NodeType, Dict[bytes32, WSWheatConnection]] = {
            NodeType.FULL_NODE: {},
            NodeType.WALLET: {},
            NodeType.HARVESTER: {},
            NodeType.FARMER: {},
        ...
        self.wheat_ca_crt_path, self.wheat_ca_key_path = wheat_ca_crt_key
    ...
    async def garbage_collect_connections_task(self) -> None:
        ...
        while True:
            ...
            to_remove: List[WSWheatConnection] = []
    ...
    async def start_server(self, on_connect: Callable = None):
        ...
        if authenticate:
            ...
            ssl_context = ssl_context_for_server(
                self.wheat_ca_crt_path, self.wheat_ca_key_path, self.p2p_crt_path, self.p2p_key_path, log=self.log
            )
    ...
    async def incoming_connection(self, request):
        ...
        if peer_id == self.node_id:
            return ws
        connection: Optional[WSWheatConnection] = None
        try:
            connection = WSWheatConnection(
                self._local_type,
                ws,
                self._port,
                self.log,
    ...
    async def connection_added(self, connection: WSWheatConnection, on_connect=None):
    ...
    async def start_client(
        ...
        if auth:
            ...
            ssl_context = ssl_context_for_client(
                self.wheat_ca_crt_path, self.wheat_ca_key_path, self.p2p_crt_path, self.p2p_key_path
            )
        session = None
        connection: Optional[WSWheatConnection] = None
        try:
    ...
    def connection_closed(self, connection: WSWheatConnection, ban_time: int):
    ...
    async def incoming_api_task(self) -> None:
        ...
        while True:
            ...
            async def api_call(full_message: Message, connection: WSWheatConnection, task_id):
    ...
    def get_outgoing_connections(self) -> List[WSWheatConnection]:
        result = []
        for _, connection in self.all_connections.items():
    ...
    def get_full_node_outgoing_connections(self) -> List[WSWheatConnection]:
        result = []
    ...
    async def get_peer_info(self) -> Optional[PeerInfo]:
        ...
        try:
            ...
            async with ClientSession(timeout=timeout) as session:
                async with session.get("https://ip.wheat.network/") as resp:
                    if resp.status == 200:
    ...
    def is_trusted_peer(self, peer: WSWheatConnection, trusted_peers: Dict) -> bool:
