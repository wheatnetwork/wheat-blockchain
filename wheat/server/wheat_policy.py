...
async def _wheat_create_server(
    cls: Any,
    protocol_factory: _ProtocolFactory,
    host: Any,
    port: Any,
...
class WheatSelectorEventLoop(asyncio.SelectorEventLoop):
    ...
    async def create_server(self, *args, **kwargs) -> PausableServer:  # type: ignore[no-untyped-def]
        return await _wheat_create_server(super(), *args, **kwargs)


if sys.platform == "win32":

    class WheatProactor(IocpProactor):
        ...
        async def _wheat_accept_loop(self, listener: socket.socket) -> Tuple[socket.socket, Tuple[object, ...]]:
            while True:
                ...
                try:
                    return await self._wheat_accept(listener)
                except WindowsError as exc:  # pylint: disable=E0602
                    if exc.winerror not in (_winapi.ERROR_NETNAME_DELETED, _winapi.ERROR_OPERATION_ABORTED):
                        raise

        def _wheat_accept(self, listener: socket.socket) -> asyncio.Future[Tuple[socket.socket, Tuple[object, ...]]]:
            ...
            async def accept_coro(self: WheatProactor, future: asyncio.Future[object], conn: socket.socket) -> None:
        ...
        def accept(self, listener: socket.socket) -> asyncio.Future[Tuple[socket.socket, Tuple[object, ...]]]:
            coro = self._wheat_accept_loop(listener)
            return asyncio.ensure_future(coro)

    class WheatProactorEventLoop(ProactorEventLoop):
...
class WheatPolicy(asyncio.DefaultEventLoopPolicy):
    def new_event_loop(self) -> asyncio.AbstractEventLoop:
        ...
        if sys.platform == "win32":
            if sys.version_info >= (3, 8):
                ...
                loop_factory = WheatProactorEventLoop
            else:
                # separate condition so coverage can report when this is no longer used
                loop_factory = WheatSelectorEventLoop
        else:
            loop_factory = WheatSelectorEventLoop
...
def set_wheat_policy(connection_limit: int) -> None:
    ...
    asyncio.set_event_loop_policy(WheatPolicy())
