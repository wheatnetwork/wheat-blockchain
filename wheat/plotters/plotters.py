...
from wheat.plotters.bladebit import get_bladebit_install_info, plot_bladebit
from wheat.plotters.chiapos import get_chiapos_install_info, plot_wheat
from wheat.plotters.madmax import get_madmax_install_info, plot_madmax
...
def call_plotters(root_path: Path, args):
    # Add `plotters` section in WHEAT_ROOT.
    wheat_root_path = root_path
    ...
    if not os.path.exists(root_path):
        print(f"Creating plotters folder within WHEAT_ROOT: {root_path}")
        try:
            os.mkdir(root_path)
        except Exception as e:
            print(f"Cannot create plotters root path {root_path} {type(e)} {e}.")

    plotters = argparse.ArgumentParser("wheat plotters", description="Available options.")
    subparsers = plotters.add_subparsers(help="Available options", dest="plotter")

    build_parser(subparsers, root_path, chia_plotter_options, "chiapos", "Create a plot with the default wheat plotter")
    ...
    deprecation_warning = (
        "[DEPRECATED] 'wheat plotters install' is no longer available. Use install-plotter.sh/ps1 instead."
    )
    subparsers.add_parser("install", help=deprecation_warning, add_help=False)

    deprecation_warning_bb2 = "[DEPRECATED] 'wheat plotters bladebit2' was integrated to 'wheat plotters bladebit'"
    ...
    if args.plotter is None:
        ...
        plot_wheat(args, wheat_root_path)
    elif args.plotter == "madmax":
        plot_madmax(args, wheat_root_path, root_path)
    elif args.plotter == "bladebit":
        plot_bladebit(args, wheat_root_path, root_path)
    elif args.plotter == "version":
        show_plotters_version(wheat_root_path)
