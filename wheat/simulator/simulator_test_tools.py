...
from wheat.simulator.start_simulator import async_main as start_simulator_main
...
from wheat.util.config import create_default_wheat_config, load_config, save_config
...
from wheat.wallet.derive_keys import master_sk_to_wallet_sk
...
def create_config(
    ...
    # create wheat directories
    create_default_wheat_config(wheat_root)
    create_all_ssl(
        wheat_root,
    ...
    config["simulator"]["farming_address"] = encode_puzzle_hash(get_puzzle_hash_from_key(keychain, fingerprint), "twheat")
...
async def start_simulator(wheat_root: Path, automated_testing: bool = False) -> AsyncGenerator[FullNodeSimulator, None]:
    sys.argv = [sys.argv[0]]  # clear sys.argv to avoid issues with config.yaml
    started_simulator = await start_simulator_main(True, automated_testing, root_path=wheat_root)
...
async def get_full_wheat_simulator(
    wheat_root: Path,
    ...
    """
    A wheat root Path is required.
    The wheat root Path can be a temporary directory (tempfile.TemporaryDirectory)
    ...
    This test is fully interdependent and can be used without the rest of the wheat test suite.
    ...
    with Lockfile.create(daemon_launch_lock_path(wheat_root)):
        ...
        crt_path = wheat_root / config["daemon_ssl"]["private_crt"]
        key_path = wheat_root / config["daemon_ssl"]["private_key"]
        ca_crt_path = wheat_root / config["private_ssl_ca"]["crt"]
        ca_key_path = wheat_root / config["private_ssl_ca"]["key"]

        ws_server = WebSocketServer(wheat_root, ca_crt_path, ca_key_path, crt_path, key_path)
        async with SignalHandlers.manage() as signal_handlers:
            await ws_server.setup_process_global_state(signal_handlers=signal_handlers)
            async with ws_server.run():
                async for simulator in start_simulator(wheat_root, automated_testing):
                    yield simulator, wheat_root, config, mnemonic, fingerprint, keychain
