...
from wheat.cmds.beta_funcs import (
    default_beta_root_path,
    prepare_wheat_blockchain_log,
    prepare_logs,
...
from wheat.util.beta_metrics import metrics_log_interval_default
from wheat.util.config import lock_and_load_config, save_config


def print_restart_warning() -> None:
    print("\nRestart the daemon and any running wheat services for changes to take effect.")
...
def configure(ctx: click.Context, path: Optional[str], interval: Optional[int]) -> None:
    ...
    with lock_and_load_config(root_path, "config.yaml") as config:
        if "beta" not in config:
            raise click.ClickException("beta test mode is not enabled, enable it first with `wheat beta enable`")
...
def prepare_submission_cmd(ctx: click.Context) -> None:
    with lock_and_load_config(ctx.obj["root_path"], "config.yaml") as config:
        ...
        if beta_root_path is None:
            raise click.ClickException("beta test mode not enabled. Run `wheat beta enable` first.")
    ...
    wheat_blockchain_path = Path(prepare_result / "wheat-blockchain")
    wheat_logs = prepare_logs(plotting_path, prepare_wheat_blockchain_log)
    plotting_logs = prepare_logs(wheat_blockchain_path, prepare_plotting_log)
    ...
    with zipfile.ZipFile(submission_file_path, "w", zipfile.ZIP_DEFLATED) as zip_file:
        files_added = add_files(wheat_logs) + add_files(plotting_logs)

    if files_added == 0:
        submission_file_path.unlink()
        message = f"No logs files found in {str(plotting_path)!r} and {str(wheat_blockchain_path)!r}."
