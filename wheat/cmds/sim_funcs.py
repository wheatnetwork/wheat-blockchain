...
from wheat.cmds.cmds_util import get_any_service_client
from wheat.cmds.start_funcs import async_start
from wheat.consensus.coinbase import create_puzzlehash_for_pk
from wheat.server.outbound_message import NodeType
from wheat.simulator.simulator_full_node_rpc_client import SimulatorFullNodeRpcClient
...
from wheat.util.config import load_config, save_config, set_peer_info
from wheat.util.errors import KeychainFingerprintExists
...
def create_wheat_directory(
    ...
    """
    This function creates a new wheat directory and returns a heavily modified config,
    suitable for use in the simulator.
    """
    from wheat.cmds.init_funcs import wheat_init

    if not wheat_root.is_dir() or not Path(wheat_root / "config" / "config.yaml").exists():
        # create wheat directories & load config
        wheat_init(wheat_root, testnet=True, fix_ssl_permissions=True)
        config: Dict[str, Any] = load_config(wheat_root, "config.yaml")
        ...
        if not docker_mode:  # We want predictable ports for our docker image.
            ...
            config["network_overrides"]["config"]["simulator0"]["default_full_node_port"] = 21559 + port_offset
            # wallet
        ...
        config = load_config(wheat_root, "config.yaml")
    # simulator overrides
    ...
    save_config(wheat_root, "config.yaml", config)
    return config


def display_key_info(fingerprint: int, prefix: str) -> None:
    """
    Display key info for a given fingerprint, similar to the output of `wheat keys show`.
    """
...
def generate_and_return_fingerprint(mnemonic: Optional[str] = None) -> int:
    ...
    """
    from wheat.util.keychain import generate_mnemonic
...
async def generate_plots(config: Dict[str, Any], root_path: Path, fingerprint: int, bitfield: bool) -> None:
    ...
    from wheat.simulator.start_simulator import PLOT_SIZE, PLOTS

    farming_puzzle_hash = decode_puzzle_hash(config["simulator"]["farming_address"])
    os.environ["WHEAT_ROOT"] = str(root_path)  # change env variable, to make it match what the daemon would set it to
...
async def async_config_wizard(
    ...
    if fingerprint is None:
        ...
        return
    # create wheat directory & get config.
    print("Creating wheat directory & config...")
    config = create_wheat_directory(root_path, fingerprint, farming_address, plot_directory, auto_farm, docker_mode)
    ...
    sys.argv[0] = str(Path(sys.executable).parent / "wheat")  # fix path for tests
    ...
    while True:
        try:
            ...
            break
    print(f"\nMake sure your WHEAT_ROOT Environment Variable is set to: {root_path}")
...
async def print_status(
    ...
    """
    from wheat.cmds.show_funcs import print_blockchain_state
    ...
    async with get_any_service_client(SimulatorFullNodeRpcClient, rpc_port, root_path) as (node_client, config):
        ...
        # chain status ( basically wheat show -s)
        ...
        print(
            ...
            f"with a balance of: "
            f"{sum(coin_records.coin.amount for coin_records in farming_coin_records) / units['wheat']} TWHEAT."
        )
        if show_addresses:
            print("All Addresses: ")
