...
from wheat.util.keychain import set_keys_root_path, supports_keyring_passphrase
from wheat.util.ssl import check_ssl
...
@click.group(
    help=f"\n  Manage wheat blockchain infrastructure ({__version__})\n",
    epilog="Try 'wheat start node', 'wheat netspace -d 192', or 'wheat show -s'",
...
@click.option(
    ...
    default=False,
    is_flag=True,
    hidden=True,  # --wait-for-unlock is only set when launched by wheat start <service>
)
@click.pass_context
