...
from wheat.cmds.cmds_util import CMDTXConfigLoader, get_wallet_client, transaction_status_msg, transaction_submitted_msg
from wheat.cmds.units import units
from wheat.cmds.wallet_funcs import get_mojo_per_unit, get_wallet_type
...
from wheat.wallet.util.tx_config import DEFAULT_COIN_SELECTION_CONFIG
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        res = await wallet_client.create_new_dao_wallet(
            ...
            tx_config=CMDTXConfigLoader.from_json_dict({"reuse_puzhash": True}).to_tx_config(
                units["wheat"], config, fingerprint
            ),
            dao_rules=None,
            amount_of_cats=None,
...
async def create_dao_wallet(args: Dict[str, Any], wallet_rpc_port: Optional[int], fp: int) -> None:
    proposal_minimum = uint64(int(Decimal(args["proposal_minimum_amount"]) * units["wheat"]))
    ...
    final_fee: uint64 = uint64(int(fee * units["wheat"]))

    fee_for_cat = Decimal(args["fee_for_cat"])
    final_fee_for_cat: uint64 = uint64(int(fee_for_cat * units["wheat"]))
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        ...
        if len(conf_coins) < 2:  # pragma: no cover
            raise ValueError("DAO creation requires at least 2 wheat coins in your wallet.")
        res = await wallet_client.create_new_dao_wallet(
            mode="new",
            dao_rules=dao_rules,
...
async def get_treasury_balance(args: Dict[str, Any], wallet_rpc_port: Optional[int], fp: int) -> None:
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, _, _):
        ...
        wheat_mojos = get_mojo_per_unit(WalletType.STANDARD_WALLET)
        cat_mojos = get_mojo_per_unit(WalletType.CAT)
        for asset_id, balance in balances.items():
            if asset_id == "wheat":
                print(f"WHEAT: {balance / wheat_mojos}")
            else:
...
async def show_proposal(args: Dict[str, Any], wallet_rpc_port: Optional[int], fp: int) -> None:
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, _, config):
        ...
        if ptype == "spend":
            wheat_conds = pd["wheat_conditions"]
            asset_conds = pd["asset_conditions"]
            print("")
            if wheat_conds:
                print("Proposal WHEAT Conditions")
                for pmt in wheat_conds:
...
async def vote_on_proposal(args: Dict[str, Any], wallet_rpc_port: Optional[int], fp: int) -> None:
    ...
    fee = args["fee"]
    final_fee: uint64 = uint64(int(Decimal(fee) * units["wheat"]))
    proposal_id = args["proposal_id"]
...
async def create_spend_proposal(args: Dict[str, Any], wallet_rpc_port: Optional[int], fp: int) -> None:
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        ...
        if res["success"]:
            asset_id_name = asset_id if asset_id else "WHEAT"
