...
from wheat.plotting.util import add_plot_directory, validate_plot_size

log = logging.getLogger(__name__)


def show_plots(root_path: Path) -> None:
    from wheat.plotting.util import get_plot_directories

    print("Directories where plots are being searched for:")
    print("Note that subdirectories must be added manually")
    print(
        "Add with 'wheat plots add -d [dir]' and remove with"
        + " 'wheat plots remove -d [dir]'"
        + " Scan and check plots with 'wheat plots check'"
    )
    print()
...
def plots_cmd(ctx: click.Context) -> None:
    ...
    if not root_path.is_dir():
        raise RuntimeError("Please initialize (or migrate) your config directory with 'wheat init'")
...
def create_cmd(
    ...
    from wheat.plotting.create_plots import create_plots, resolve_plot_keys
    from wheat.plotting.util import Params
...
def check_cmd(
    ...
    from wheat.plotting.check_plots import check_plots
...
def remove_cmd(ctx: click.Context, final_dir: str) -> None:
    from wheat.plotting.util import remove_plot_directory
