...
from wheat.cmds.passphrase_funcs import get_current_passphrase
from wheat.daemon.client import DaemonProxy, connect_to_daemon_and_validate
from wheat.util.keychain import Keychain, KeyringMaxUnlockAttempts
from wheat.util.service_groups import services_for_groups
...
def launch_start_daemon(root_path: Path) -> subprocess.Popen:
    ...
    wheat = sys.argv[0]
    process = subprocess.Popen(f"{wheat} run_daemon --wait-for-unlock".split(), stdout=subprocess.PIPE)
    return process
...
async def async_start(root_path: Path, group: str, restart: bool) -> None:
    ...
    if daemon is None:
        print("Failed to create the wheat daemon")
        return None
