...
from wheat.ssl.create_ssl import generate_ca_signed_cert, get_wheat_ca_crt_key, make_ca_cert
...
def check_keys(new_root: Path) -> None:
    ...
    for i in range(number_of_ph_to_search):
        ...
        for sk, _ in all_sks:
            ...
            if all_targets[-1] == config["farmer"].get("wheat_target_address"):
                stop_searching_for_farmer = True
            if all_targets[-1] == config["pool"].get("wheat_target_address"):
                stop_searching_for_pool = True

    # Set the destinations
    if "wheat_target_address" not in config["farmer"]:
        print(f"Setting the wheat destination address for coinbase fees reward to {all_targets[0]}")
    ...
    if "wheat_target_address" not in config["pool"]:
        print(f"Setting the wheat destination address for coinbase reward to {all_targets[0]}")
...
def create_all_ssl(root: Path):
    ...
    wheat_ca_crt_path.write_bytes(wheat_ca_crt)
    wheat_ca_key_path.write_bytes(wheat_ca_key)
