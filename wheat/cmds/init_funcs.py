...
from wheat.ssl.create_ssl import (
    ensure_ssl_dirs,
    generate_ca_signed_cert,
    get_wheat_ca_crt_key,
    make_ca_cert,
...
def check_keys(new_root: Path, keychain: Optional[Keychain] = None) -> None:
    ...
    if len(all_sks) == 0:
        print("No keys are present in the keychain. Generate them with 'wheat keys generate'")
        return None
    ...
    with get_config_lock(new_root, "config.yaml"):
        ...
        all_targets = []
        stop_searching_for_farmer = "wheat_target_address" not in config["farmer"]
        stop_searching_for_pool = "wheat_target_address" not in config["pool"]
        ...
        for i in range(number_of_ph_to_search):
            ...
            for sk, _ in all_sks:
                ...
                if all_targets[-1] == config["farmer"].get("wheat_target_address") or all_targets[-2] == config[
                    "farmer"
                ].get("wheat_target_address"):
                    stop_searching_for_farmer = True
                if all_targets[-1] == config["pool"].get("wheat_target_address") or all_targets[-2] == config["pool"].get(
                    "wheat_target_address"
                ):
                    stop_searching_for_pool = True

        # Set the destinations, if necessary
        updated_target: bool = False
        if "wheat_target_address" not in config["farmer"]:
            print(
                f"Setting the wheat destination for the farmer reward (1/8 plus fees, solo and pooling)"
                f" to {all_targets[0]}"
            )
            config["farmer"]["wheat_target_address"] = all_targets[0]
            updated_target = True
        elif config["farmer"]["wheat_target_address"] not in all_targets:
            print(
                f"WARNING: using a farmer address which we might not have the private"
                f" keys for. We searched the first {number_of_ph_to_search} addresses. Consider overriding "
                f"{config['farmer']['wheat_target_address']} with {all_targets[0]}"
            )

        if "pool" not in config:
            config["pool"] = {}
        if "wheat_target_address" not in config["pool"]:
            print(f"Setting the wheat destination address for pool reward (7/8 for solo only) to {all_targets[0]}")
            config["pool"]["wheat_target_address"] = all_targets[0]
            updated_target = True
        elif config["pool"]["wheat_target_address"] not in all_targets:
            print(
                f"WARNING: using a pool address which we might not have the private"
                f" keys for. We searched the first {number_of_ph_to_search} addresses. Consider overriding "
                f"{config['pool']['wheat_target_address']} with {all_targets[0]}"
            )
        if updated_target:
            print(
                f"To change the WHEAT destination addresses, edit the `wheat_target_address` entries in"
...
def create_all_ssl(root_path: Path):
    ...
    wheat_ca_crt, wheat_ca_key = get_wheat_ca_crt_key()
    wheat_ca_crt_path = ca_dir / "wheat_ca.crt"
    wheat_ca_key_path = ca_dir / "wheat_ca.key"
    write_ssl_cert_and_key(wheat_ca_crt_path, wheat_ca_crt, wheat_ca_key_path, wheat_ca_key)
    ...
    wheat_ca_crt, wheat_ca_key = get_wheat_ca_crt_key()
    generate_ssl_for_nodes(ssl_dir, wheat_ca_crt, wheat_ca_key, False, overwrite=False)
...
def init(
    ...
    if create_certs is not None:
        ...
        return wheat_init(root_path, fix_ssl_permissions=fix_ssl_permissions, testnet=testnet, v1_db=v1_db)
...
def wheat_minor_release_number():
    res = int(wheat_version_number()[2])
    print(f"Install release number: {res}")
    return res


def wheat_full_version_str() -> str:
    major, minor, patch, dev = wheat_version_number()
...
def wheat_init(
    ...
    *,
    ...
    """
    if os.environ.get("WHEAT_ROOT", None) is not None:
        print(
            f"warning, your WHEAT_ROOT is set to {os.environ['WHEAT_ROOT']}. "
            f"Please unset the environment variable and run wheat init again\n"
            f"or manually migrate config.yaml"
        )

    print(f"Wheat directory {root_path}")
    ...
    print("")
    print("To see your keys, run 'wheat keys show --show-mnemonic-seed'")
