...
from wheat.cmds.cmds_util import (
    CMDTXConfigLoader,
    cli_confirm,
    get_wallet_client,
...
from wheat.util.config import selected_network_address_prefix
...
from wheat.wallet.transaction_sorting import SortKey
            ...
            for item in [*offers, *requests]:
                ...
                offer, trade_record = await wallet_client.create_offer_for_ids(
                    ...
                    tx_config=CMDTXConfigLoader(
                        reuse_puzhash=reuse_puzhash,
                    ).to_tx_config(units["wheat"], config, fingerprint),
                )
                if offer is not None:
                    ...
                    print(
                        f"Use wheat wallet get_offers --id " f"{trade_record.trade_id} -f {fingerprint} to view status"
                    )
                else:
...
async def print_offer_summary(
    cat_name_resolver: CATNameResolver, sum_dict: Dict[str, int], has_fee: bool = False, network_wheat: str = "WHEAT"
) -> None:
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        ...
        if not examine_only:
            ...
            trade_record = await wallet_client.take_offer(
                offer,
                fee=fee,
                tx_config=CMDTXConfigLoader().to_tx_config(units["wheat"], config, fingerprint),
            )
...
async def cancel_offer(
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        ...
        await wallet_client.cancel_offer(
            offer_id, CMDTXConfigLoader().to_tx_config(units["wheat"], config, fingerprint), secure=secure, fee=fee
        )
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        try:
            response = await wallet_client.did_message_spend(
                ...
                coin_announcements,
                CMDTXConfigLoader().to_tx_config(units["wheat"], config, fingerprint),
            )
    ...
    *,
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        royalty_address = (
            ...
            else ensure_valid_address(royalty_address, allowed_types={AddressType.WHEAT}, config=config)
        )
        target_address = (
            None
            if not target_address
            else ensure_valid_address(target_address, allowed_types={AddressType.WHEAT}, config=config)
        )
        fee: int = int(d_fee * units["wheat"])
        try:
...
async def transfer_nft(
    *,
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        try:
            target_address = ensure_valid_address(target_address, allowed_types={AddressType.WHEAT}, config=config)
            fee: int = int(d_fee * units["wheat"])
...
async def send_notification(
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        ...
        amount: uint64 = uint64(d_amount * units["wheat"])
        message: bytes = bytes(message_hex, "utf8")
        fee: uint64 = uint64(d_fee * units["wheat"])
...
async def sign_message(
    *,
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        if addr_type == AddressType.WHEAT:
            if address is None:
                print("Address is required for WHEAT address type.")
                return
    ...
    *, wallet_rpc_port: Optional[int], fp: Optional[int], fee: Decimal, tx_ids_str: str, force: bool = False
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, _, _):
        ...
        if fee < 0:
            ...
            return
        response = await wallet_client.spend_clawback_coins(tx_ids, int(fee * units["wheat"]), force)
        print(str(response))
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        vc_record, txs = await wallet_client.vc_mint(
            ...
            if target_address is None
                ...
                ensure_valid_address(target_address, allowed_types={AddressType.WHEAT}, config=config)
            ),
            uint64(int(d_fee * units["wheat"])),
        )
    ...
    *,
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        txs = await wallet_client.vc_spend(
            ...
            fee=uint64(int(d_fee * units["wheat"])),
            tx_config=CMDTXConfigLoader(
                reuse_puzhash=reuse_puzhash,
            ).to_tx_config(units["wheat"], config, fingerprint),
        )
...
async def approve_r_cats(
    ...
    async with get_wallet_client(wallet_rpc_port, fingerprint) as (wallet_client, fingerprint, config):
        ...
        txs = await wallet_client.crcat_approve_pending(
            ...
            fee=uint64(int(fee * units["wheat"])),
            tx_config=CMDTXConfigLoader(
...
async def recover_pool_nft(
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, _):
        final_fee: uint64 = uint64(int(fee * units["ball"]))
