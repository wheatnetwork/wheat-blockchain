...
from wheat.data_layer.data_layer_errors import LauncherCoinNotFoundError, OfferIntegrityError
from wheat.data_layer.data_layer_util import OfferStore, ProofOfInclusion, ProofOfInclusionLayer, StoreProofs, leaf_hash
...
from wheat.wallet.db_wallet.db_wallet_puzzles import (
    ACS_MU,
    ACS_MU_PH,
    GRAFTROOT_DL_OFFERS,
    SINGLETON_LAUNCHER,
...
from wheat.wallet.puzzles.singleton_top_layer_v1_1 import SINGLETON_LAUNCHER_HASH
from wheat.wallet.trading.offer import NotarizedPayment, Offer
from wheat.wallet.transaction_record import TransactionRecord
from wheat.wallet.util.compute_memos import compute_memos
from wheat.wallet.util.merkle_utils import _simplify_merkle_proof
from wheat.wallet.util.transaction_type import TransactionType
from wheat.wallet.util.tx_config import CoinSelectionConfig, TXConfig, TXConfigLoader
from wheat.wallet.util.wallet_sync_utils import fetch_coin_spend, fetch_coin_spend_for_coin_state
...
class DataLayerWallet:
    ...
    async def get_launcher_coin_state(self, launcher_id: bytes32, peer: WSWheatConnection) -> CoinState:
    ...
    async def delete_mirror(
        ...
        if excess_fee > 0:
            [wheat_tx] = await self.wallet_state_manager.main_wallet.generate_signed_transaction(
                uint64(1),
                new_puzhash,
                tx_config,
            ...
            txs = [
                dataclasses.replace(
                    txs[0], spend_bundle=SpendBundle.aggregate([txs[0].spend_bundle, wheat_tx.spend_bundle])
                ),
                dataclasses.replace(wheat_tx, spend_bundle=None),
            ]
    ...
    async def coin_added(self, coin: Coin, height: uint32, peer: WSWheatConnection, coin_data: Optional[object]) -> None:
