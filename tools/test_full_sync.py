...
from wheat.simulator.block_tools import make_unfinished_block
...
async def run_sync_test(
    ...
    with tempfile.TemporaryDirectory() as root_dir:
        ...
        wheat_init(root_path, should_check_keys=False, v1_db=(db_version == 1))
        ...
        full_node.set_server(cast(WheatServer, FakeServer()))
        async with full_node.manage():
            ...
            peer: WSWheatConnection = cast(WSWheatConnection, FakePeer())
...
async def run_sync_checkpoint(
    ...
    wheat_init(root_path, should_check_keys=False, v1_db=False)
    config = load_config(root_path, "config.yaml")
    ...
    async with full_node.manage():
        peer: WSWheatConnection = FakePeer()  # type: ignore[assignment]
