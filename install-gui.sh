...
if [ -z "$VIRTUAL_ENV" ]; then
  echo "This requires the wheat python virtual environment."
  echo "Execute '. ./activate' before running."
  exit 1
fi

if [ "$(id -u)" = 0 ]; then
  echo "The Wheat Blockchain GUI can not be installed or run by the root user."
  exit 1
fi

# Allows overriding the branch or commit to build in wheat-blockchain-gui
SUBMODULE_BRANCH=$1
...
if [ ! "$CI" ]; then
  ...
  # Set modified output of `wheat version` to version property of GUI's package.json
...
echo ""
echo "Wheat blockchain install-gui.sh completed."
echo ""
