...
install_python3_and_sqlite3_from_source_with_yum() {
  ...
  cd "$TMP_PATH"
  # Install sqlite>=3.37
  # yum install sqlite-devel brings sqlite3.7 which is not compatible with wheat
  ...
  # yum install python3 brings Python3.6 which is not supported by wheat
  cd ..
      ...
      if [ "$BEST_VERSION" = "" ]; then
        ...
        if [ "$BEST_VERSION" = "3" ]; then
          ...
          if [[ "$PY3_VERSION" =~ 3.11.* ]]; then
            echo "Wheat requires Python version < 3.11.0" >&2
...
echo ""
echo "Wheat blockchain install.sh complete."
echo "For assistance join us on Keybase in the #support chat channel:"
echo "https://keybase.io/team/wheat_network.public"
echo ""
...
echo ""
echo "Type '. ./activate' and then 'wheat init' to begin."
