...
if [ "$REDHAT_PLATFORM" = "x86_64" ]; then
	...
	# Disables build links from the generated rpm so that we dont conflict with other packages. See https://github.com/WheatNetwork/wheat-blockchain/issues/3846
  ...
  --license ../LICENSE --options.bin wheat-blockchain --options.name wheat-blockchain
  LAST_EXIT_CODE=$?
  if [ "$LAST_EXIT_CODE" -ne 0 ]; then
