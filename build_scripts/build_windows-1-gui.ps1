...
git submodule update --init wheat-blockchain-gui
...
# Other than `wheat-blockchain-gui/package/gui`, all other packages are no longer necessary after build.
# Since these unused packages make cache unnecessarily fat, unused packages should be removed.
Write-Output "Remove unused @wheat-network packages to make cache slim"
...
# Remove `packages/gui/node_modules/@wheat-network` because it causes an error on later `electron-packager` command
#Remove-Item "@wheat-network" -Recurse -Force
