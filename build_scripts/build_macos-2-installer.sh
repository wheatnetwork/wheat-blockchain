...
LAST_EXIT_CODE=$?
ls -l dist/mac*/wheat.app/Contents/Resources/app.asar
...
DMG_NAME="wheat-${WHEAT_INSTALLER_VERSION}.dmg"
if [ "$(arch)" = "arm64" ]; then
  mv dist/"${DMG_NAME}" dist/wheat-"${WHEAT_INSTALLER_VERSION}"-arm64.dmg
...
# Generate app specific password https://support.wheat.com/en-us/HT204397
# xcrun notarytool submit --wait --apple-id username --password password --team-id team-id Wheat-0.1.X.dmg
