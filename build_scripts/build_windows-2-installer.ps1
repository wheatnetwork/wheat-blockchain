...
Set-Location -Path "wheat-blockchain-gui\packages\gui" -PassThru
...
npx electron-builder build --win --x64 --config.productName="Wheat" --dir
...
npx electron-builder build --win --x64 --config.productName="Wheat" --pd ".\dist\win-unpacked"
Write-Output "   ---"

If ($env:HAS_SIGNING_SECRET) {
   Write-Output "   ---"
   Write-Output "Sign Final Installer App"
   signtool.exe sign /sha1 $env:SM_CODE_SIGNING_CERT_SHA1_HASH /tr http://timestamp.digicert.com /td SHA256 /fd SHA256 .\dist\WheatSetup-$packageVersion.exe
   Write-Output "   ---"
   Write-Output "Verify signature"
   Write-Output "   ---"
   signtool.exe verify /v /pa .\dist\WheatSetup-$packageVersion.exe
}   Else    {
   Write-Output "Skipping verify signatures - no authorization to install certificates"
}
...
Write-Output "   ---"
Copy-Item ".\dist\win-unpacked" -Destination "$env:GITHUB_WORKSPACE\wheat-blockchain-gui\Wheat-win32-x64" -Recurse
mkdir "$env:GITHUB_WORKSPACE\wheat-blockchain-gui\release-builds\windows-installer" -ea 0
Copy-Item ".\dist\WheatSetup-$packageVersion.exe" -Destination "$env:GITHUB_WORKSPACE\wheat-blockchain-gui\release-builds\windows-installer"
